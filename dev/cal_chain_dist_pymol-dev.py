...
def find_contacts_between_two_chains(
    ...
    print_msg(f"Polar contacts: {polar_contacts}", "debug")
...
# PyMOL
# ----------------------------------------
# a dev version of the select_interface function
def select_interface(
    ...
    for c1, c2 in chain_pairs:
        ...
        # HBond distance
        # --------------------
        def create_dist_selection_str(atom_pair_list: List[Tuple[str, str]], obj_name: str) -> List[str]:
        ...
        if results["hbond_atom_pairs"]:
            dist_sel = create_dist_selection_str(atom_pair_list=results["hbond_atom_pairs"], obj_name=selection)
            # pymol selection
        ...
        if results["polar_atom_pairs"]:
            dist_sel = create_dist_selection_str(atom_pair_list=results["polar_atom_pairs"], obj_name=selection)
